...
///
/// `--directory` обязательный: корень песочницы. `--host`/`--port`
/// override'ят YAML (если заданы), а не заменяют его. `--client-pass`
/// включает bearer-аутентификацию; без него на non-loopback интерфейсе
/// демон отказывается подниматься (это ловушка «случайно открыл порт в
/// сеть без пароля»).
#[derive(Debug, Clone, Args)]
pub struct ServerArgs {
    /// Корень, внутри которого разрешено создавать файлы. Канонизируется
    /// на старте — симлинк в корне разрешается до реального пути.
    #[arg(long)]
