//! Конфигурация демона — YAML-файл в платформо-зависимом config-каталоге
//! (см. [`brook_runtime::AppPaths`]): на macOS `~/Library/Application
//! Support/brook/brook.yaml`, на Linux `~/.config/brook/brook.yaml`.
//! Статическая конфигурация процесса: на лету не перечитывается, правки
//! требуют рестарта демона.
//!
//! ## Модель ключей
//!
...
/// Имя конфиг-файла. Каталог выбирается резолвером
/// [`brook_runtime::AppPaths`].
