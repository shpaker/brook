...
//! `mpsc::Sender<FileCommand>` читается как «канал для команд», а
//! не как «канал со строками, которые мы договорились интерпретировать».

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum FileCommand {
    /// Приостановить активный файл: останавливаем воркеры, но
...
mod tests {
    ...
    fn copy_is_implemented() {
        ...
        fn take(_c: FileCommand) {}
        let c = FileCommand::Pause;
        take(c);
        // Если бы `FileCommand` не был `Copy`, эта строка упала бы с
        // "value used after move".
        assert_eq!(c, FileCommand::Pause);
    }
}
