...
- **TUI модалки — структура и клавиатурный контракт.** Все модалки — `Rounded`-рамка 62×6: заголовок `[  title  ]` в верхней рамке, действия `[  yes  |  no  ]` или хинт `[  Tab · switch  |  Enter · add  |  Esc · cancel  ]` в нижней; содержимое — только текст (4 строки: пустая · текст · текст · пустая). Хелперы: `bottom_yes_no`, `hint_line`, `modal_block` в [crates/brook-tui/src/ui/modal.rs](crates/brook-tui/src/ui/modal.rs).

  Клавиатурный контракт, единый для всех модалок и **не указываемый явно в подсказках**:
  - `Esc` — всегда эквивалент `no` (закрыть без действия).
  - `Enter` — подтвердить сфокусированную кнопку (может быть `yes` или `no`).
  - `Tab` — переключить фокус между кнопками (только в двухкнопочных модалках).
