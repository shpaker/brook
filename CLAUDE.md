...
- **TUI модалки — структура и клавиатурный контракт.** Все модалки — `Rounded`-рамка 62×6, рамка `Color::DarkGray` (как у главного окна): заголовок `[  title  ]` в верхней рамке, действия в нижней; содержимое — только текст (4 строки: пустая · текст · текст · пустая). Хелперы: `bottom_yes_no`, `hint_line`, `modal_block` в [crates/brook-tui/src/ui/modal.rs](crates/brook-tui/src/ui/modal.rs).
  ...
  Кнопки `yes`/`no` в нижней рамке рендерятся через `word_with_key` (первая буква — accent-шоткат, остаток — dim) для **сфокусированной** кнопки; несфокусированная — полностью dim. Цветовой блок REVERSED не используется. Первая буква — реальный шоткат: `y`/`n` срабатывают наравне с Tab+Enter.

  Клавиатурный контракт, единый для всех модалок и **не указываемый явно в подсказках**:
  - `Esc` и `n`/`N` — всегда эквивалент `no` (закрыть без действия).
  - `y`/`Y` — всегда эквивалент `yes` (подтвердить основное действие).
  - `Enter` — подтвердить сфокусированную кнопку (может быть `yes` или `no`).
  - `Tab` — переключить фокус между кнопками.
