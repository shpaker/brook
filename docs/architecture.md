...
Алгоритм: делим общий размер файла на целевое число кусков (`piece_target_count`, дефолт 128), округляем вверх до ближайшей степени двойки, клампим в диапазон `[piece_size_min, piece_size_max]` (дефолты 16 MiB и 128 MiB). Все три параметра живут в таблице `settings` (см. [Конфигурация](#конфигурация)) и позволяют пользователю сместить баланс в сторону более точного ресюма (меньшие границы) или меньшего числа Range-запросов (большие границы).
...
5. **`piece_size` фиксируется при создании загрузки** и хранится в `meta` таблице `.index.brook`. Ресюм детерминирован даже при смене настроек в `settings` между запусками.
