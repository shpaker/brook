# Архитектура

## Принципы
- Движок загрузок (`brook-core`) не знает про UI и API — чистая библиотека.
- UI и удалённые клиенты общаются с движком **только через gRPC**.
- Каждая активная загрузка — отдельный актор (tokio-task). Над ними — единый менеджер.
- RAM-бюджет на одну загрузку — константный; запись на диск идёт кусками.

## Структура workspace

```
brook/                            # workspace root
├── Cargo.toml                    # [workspace]
├── proto/
│   └── brook/v1/brook.proto      # единый источник правды контракта API
├── crates/
│   ├── brook-proto/              # build.rs → prost + tonic stubs
│   ├── brook-core/               # гексагональное ядро: domain + ports (+ services); без сети и диска
│   ├── brook-http/               # HTTP-адаптер: reqwest + middleware, реализует HTTP-порты ядра
│   ├── brook-api/                # gRPC-сервер (tonic), обёртка над core + bearer-auth interceptor
│   ├── brook-runtime/            # общие для демона и TUI примитивы: endpoint-файл, константы
│   ├── brook-daemon/             # lib: серверный стек (конфиг, storage-адаптеры, bootstrap)
│   ├── brook-tui/                # lib: ratatui gRPC-клиент
│   └── brook/                    # единственный [[bin]] — диспетч `brook` / `brook server`
└── docs/
```

## Процессная модель

Единственный бинарь `brook` со clap-диспетчером. Без подкоманды он — TUI-клиент; `brook server …` поднимает демона. Процессов по-прежнему два (демон и клиент); общение — только gRPC. Это не оптимизация на будущее, а условие целостности контракта: UI равноправен с любым другим клиентом.

### Режимы запуска

- **`brook server --directory <DIR> [--host H] [--port P] [--client-pass …]`** — демон. `--directory` обязателен: это корень песочницы (см. [Песочница](#песочница-target_dir)). `--host`/`--port` перекрывают YAML; при `port = 0` ядро выбирает свободный порт, и актуальный адрес пишется в `.brook.endpoint` для авто-обнаружения клиентом. `--client-pass` включает bearer-аутентификацию (см. [Аутентификация](#аутентификация-bearer)); пароль можно передать через env `BROOK_CLIENT_PASS`, чтобы он не оседал в истории шелла.
- **`brook`** — TUI к локальному демону. Клиент ищет `.brook.endpoint` в CWD (адрес + порт), падает на дефолтный loopback-порт, если файла нет. Если демон не отвечает — `brook` поднимает его сам через `current_exe() server --directory <default>` (без `--port` — ephemeral bind + sidecar endpoint). Флаг `spawned_self` запоминает, что демон наш: при выходе модалка `QuitConfirm` предлагает остановить его вместе с TUI (см. [docs/ux.md](ux.md)).
- **`brook --remote HOST:PORT [--pass …]`** — TUI к удалённому демону. Локального автоспавна нет. `--pass` обязателен, если сервер поднят с `--client-pass`; если пароль не передан, TUI запрашивает его на TTY через `rpassword` до первого коннекта. На выходе ничего не гасится — чужим демоном клиент не распоряжается.

### `brook server` (демон)
1. Берёт single-instance lock на `.brook.lock` в CWD (флок).
2. Читает `./brook.yaml` (при первом запуске создаёт файл с дефолтами).
3. Открывает `./brook.db` (SQLite) и прогоняет миграции очереди.
4. Применяет настройки из YAML + CLI-override (`--host`/`--port`/`--client-pass`) в `DownloadManager` и `brook-api`; канонизирует `--directory` и сохраняет как sandbox-root.
5. Отказывается биндиться на не-loopback адрес без `--client-pass` — см. [Аутентификация](#аутентификация-bearer).
6. Поднимает `DownloadManager` (`brook-core`) — асинхронная задача.
7. Запускает gRPC-сервер (`brook-api`) с `AuthInterceptor` на `<host>:<port>`, пишет актуальный адрес в `.brook.endpoint` (атомарно через tempfile+rename).
8. Работает до `SIGTERM` / `SIGINT`; на graceful shutdown удаляет `.brook.endpoint` и отпускает `.brook.lock`.

### `brook` (TUI-режим)
1. Разрешает адрес: `--remote` > sidecar `.brook.endpoint` > дефолтный loopback-порт.
2. Если нужен пароль (remote без `--pass`, либо демон вернул `Unauthenticated` на probe) — спрашивает у TTY.
3. Unary `GetSettings` как probe. Успех → продолжаем. `Unauthenticated` → prompt + retry. Transport-ошибка на локали → автоспавн (`brook server`) + поллинг `.brook.endpoint` до ~3 s.
4. Открывает `tonic::Channel`, оборачивает `InterceptedService` c `BearerInterceptor` (пароль уходит в каждый вызов в metadata `authorization: bearer …`).
5. Рендерит ratatui-UI, который является gRPC-клиентом к демону.

**UI никогда не дёргает `DownloadManager` напрямую** — только через API. Завтра любой другой клиент (локальный или удалённый) работает по тому же контракту.

## Архитектурный стиль `brook-core`

Ядро следует паттерну **Hexagonal Architecture** (он же Ports & Adapters). Граница «чистое/грязное» совпадает с границей крейта: всё, что внутри `brook-core`, не знает про сеть, диск и ОС; всё, что снаружи, — знает.

- **Domain** — сущности и value-объекты: идентификатор загрузки, спецификация задачи (url, target_dir, воркеры), конечный автомат состояний, снимок прогресса, агрегат `Download` (spec + runtime-поля), события от engine к подписчикам и команды от клиента к engine. Это чистые типы без I/O и без зависимостей на внешний мир.
- **Ports** — трейты, через которые ядро обращается наружу: [`TPieceStorage`](#абстракция-хранилища-tpiecestorage) и `TPieceStorageFactory` для хранения кусков, [`TQueueStore`](#абстракция-очереди-queuestore) для персистентности очереди, `THttpInspect` и `TRangeFetch` для сетевого ввода-вывода. Это *outbound*-порты: ядро — клиент (вызывает методы), адаптер — сервер (реализует).
- **Services** (появятся на этапе 1.3+) — application-координаторы [`DownloadManager`](#downloadmanager-один-на-процесс) и [`DownloadEngine`](#downloadengine-один-на-загрузку). Используют domain и ports, сами I/O не трогают.
- **Adapters** живут **вне** `brook-core`: SQLite-реализации (`SqliteTQueueStore`, `LocalPieceStorage`) — в `brook-daemon`; HTTP-адаптер (`HttpInspectClient`, `RangeFetchClient`) — в отдельном крейте `brook-http`, `brook-daemon` подключает его как зависимость; gRPC-адаптер — в `brook-api`. Тестовые in-memory адаптеры (`MemoryTQueueStore`, `MemoryPieceStorage`) — в test utils самого `brook-core`, но отдельным модулем, не подмешанным в продовый код.

Что это даёт: ядро тестируется без сети и без диска (порты подменяются in-memory или `wiremock`-адаптерами); смена бэкенда хранилища или HTTP-клиента не требует изменений в ядре; статическая гарантия — если файл лежит в `brook-core`, у него нет зависимости на `rusqlite` или `reqwest` (проверяется через `cargo tree -p brook-core`).

## Модель акторов в `brook-core`

### `DownloadEngine` (один на загрузку)
Владеет всем для жизненного цикла одной загрузки:
- Воркеры (N tokio-задач), их прогресс и состояние.
- **`TPieceStorage`** — абстракция записи и индексирования кусков (см. [ниже](#абстракция-хранилища-tpiecestorage)). RAM-бюджет задаётся буфером 64–256 KB; кусок — 1–4 MB. `DownloadEngine` параметризован типом `S: TPieceStorage` и не знает про диск или S3.
- Retry-логика с экспоненциальным бэкофом.
- История смен состояний.

- Вход: mpsc-канал команд (`Pause`, `Resume`, `Cancel`).
- Выход: broadcast-канал событий (`Progress`, `StateChanged`, `WorkerUpdate`, `Completed`, `Failed`).

### `DownloadManager` (один на процесс)
- Реестр `DownloadEngine`-ов (по `DownloadId`).
- Очередь: сколько задач активно, кто ждёт.
- Принимает команды из `brook-api` и роутит в нужный engine.
- Агрегирует события всех engines в один broadcast-поток для API.
- Получает `TPieceStorageFactory` при инициализации — создаёт экземпляр `TPieceStorage` для каждой новой загрузки.
- Получает `TQueueStore` при инициализации — не знает, где и как хранится очередь (см. [ниже](#абстракция-очереди-queuestore)).

`brook-core` — **чистая библиотека**: нет зависимостей на БД, файловую систему или ОС. Всё I/O-специфичное скрыто за трейтами и живёт снаружи (в `brook-daemon`).

### Почему именно так
Один `DownloadEngine` на загрузку, один менеджер на процесс — стандартная актор-модель: каждый engine изолирован, тестируется отдельно, падает независимо. Менеджер — единственная точка координации. `TPieceStorage` и `TQueueStore` как трейты позволяют переиспользовать `brook-core` без привязки к диску, SQLite или конкретной ОС.

## Абстракция хранилища (`TPieceStorage`)

`DownloadEngine` работает с кусками через трейт `TPieceStorage`, не зная деталей реализации. Трейт описывает пять операций: запись порции байт куска (`write_piece_bytes`), атомарный коммит одного завершённого куска (`commit_done`), получение списка незавершённых кусков для возобновления (`pending_pieces`), финализацию загрузки (`finalize`), прерывание с удалением артефактов (`abort`). Инвариант `commit_done`: после возврата `Ok` байты гарантированно сохранены.

`DownloadManager` принимает фабрику `TPieceStorageFactory`, которая создаёт экземпляр `TPieceStorage` для каждой новой загрузки.

### Реализации `TPieceStorage`

| Реализация | Где живёт | Назначение |
|---|---|---|
| `LocalPieceStorage` | `brook-daemon` | Основная: `pwrite` в `.data.brook`; карта piece'ов — в общей `brook.db` (таблица `pieces`) |
| `MemoryPieceStorage` | `brook-core` (test utils) | In-memory, для юнит-тестов без диска |
| `S3PieceStorage` | внешний крейт | S3 multipart upload; локальный SQLite хранит ETag и part number |

Детали локальной реализации (`LocalPieceStorage`) — в разделе [Раскладка на диске](#раскладка-на-диске) ниже.

## Абстракция очереди (`TQueueStore`)

`DownloadManager` хранит очередь через трейт `TQueueStore` — без знания о SQLite или любой другой БД. Трейт описывает четыре операции: загрузку всех записей при старте (`load_all`), добавление новой загрузки (`insert`), обновление состояния (`update_state`), удаление записи (`remove`).

### Реализации `TQueueStore`

| Реализация | Где живёт | Назначение |
|---|---|---|
| `SqliteFileRepository` | `brook-daemon` | Основная: таблицы `files` / `file_settings` / `state_changes` в `./brook.db` (см. [Схема `brook.db`](#схема-brookdb)) |
| `MemoryTQueueStore` | `brook-core` (test utils) | In-memory, для юнит-тестов без диска |

## Раскладка на диске

> Этот раздел описывает детали `LocalPieceStorage` — реализации `TPieceStorage` по умолчанию. Другие реализации (S3 и т. д.) используют тот же трейт, но хранят данные иначе.

На каждую активную загрузку — **один артефакт** рядом в целевой папке и строки в общей `./brook.db`.

### Артефакты

1. **`<filename>.data.brook`** — пре-аллоцированный файл полного финального размера. На macOS — `F_PREALLOCATE` (реальная резервация физических блоков) + `ftruncate` до целевого размера, без явного обнуления. Куски пишутся прямо сюда через `pwrite` по своим offset'ам.
2. **Строки в `./brook.db`** (таблицы `files` / `file_settings` / `state_changes` / `pieces`) — URL, целевая папка, `total_size`, `piece_size`, история переходов состояний, статус каждого piece'а (`pending` / `done`). Подробнее — [Схема `brook.db`](#схема-brookdb).

### Кусок vs буфер записи vs воркер

- **Кусок** — единица Range-запроса и строка в индексе. Размер фиксирован при старте, подбирается под общий размер файла по правилам из раздела [Выбор `piece_size`](#выбор-piece_size).
- **Read buffer** — сколько держим в RAM во время приёма тела Range-ответа. 64–256 KB. Это и есть ручка константной RAM: `buffer × число воркеров` — верхняя оценка пикового RAM на загрузку.
- **Воркер** — параллельная HTTP-Range-задача. Воркеры делят куски через общий atomic-счётчик «следующий `pending`» (work-stealing): свободный воркер берёт следующий кусок.

### Выбор `piece_size`

При создании загрузки `piece_size` подбирается адаптивно под общий размер файла и фиксируется на весь жизненный цикл — пишется в `file_settings` в `./brook.db` и при ресюме читается оттуда, а не пересчитывается.

Алгоритм: делим общий размер файла на целевое число кусков (`piece_target_count`, дефолт 128), округляем вверх до ближайшей степени двойки, клампим в диапазон `[piece_size_min, piece_size_max]` (дефолты 16 MiB и 128 MiB). Все три параметра живут только в `brook.yaml` (см. [Конфигурация](#конфигурация)) — клиент через `DownloadSpec` их не переопределяет: нарезка — решение демона.

С дефолтными границами получаются три режима:

- **До 2 GiB** — куски равны `piece_size_min = 16 MiB`, число кусков растёт линейно с размером файла до 128. Ресюм максимально точный в рамках инвариантов ниже.
- **2–16 GiB** — рабочая зона: число кусков держится на `piece_target_count = 128`, сам кусок удваивается с удвоением файла (16 → 32 → 64 → 128 MiB).
- **≥32 GiB** — упираемся в `piece_size_max = 128 MiB`, число кусков снова растёт линейно. На таких объёмах куски мельче бессмысленны (тысячи запросов на файл), крупнее — долгий ретрай при обрыве.

Инварианты:

1. **`piece_size` — степень двойки.** Предсказуемая арифметика offset'ов и простое выравнивание.
2. **`piece_size ≥ piece_size_min` (дефолт 16 MiB).** Амортизация TLS handshake и Range-заголовков на каждом запросе. Кратность 4 KiB получается автоматически — `pwrite` ложится на страницы ФС без read-modify-write.
3. **`piece_size ≤ piece_size_max` (дефолт 128 MiB).** Ограничивает стоимость ретрая при обрыве соединения и объём теряемого трафика при `kill -9` (см. [Батчевый коммит](#батчевый-коммит)).
4. **Целевое число кусков `N ≤ piece_target_count` (дефолт 128).** CDN-friendly: сотни Range-запросов с одного IP не выглядят как сканер. На файлах крупнее `piece_target_count × piece_size_max` потолок размыкается — `piece_size_max` побеждает.
5. **`piece_size` фиксируется при создании загрузки** и хранится в `file_settings` в `./brook.db`. Ресюм детерминирован даже при смене настроек в `brook.yaml` между запусками.
6. **Число воркеров вычисляется автоматически** по размеру файла: `ceil(log2(ceil(MB/10)))`, ограничено сверху 12. Файлы до 10 MB получают одного воркера, очень большие — максимум 12. Параметр независим от `piece_size`: ни тот ни другой не влияет на расчёт соседа.
7. **Неизвестный `Content-Length`** (chunked, нет заголовка) — `piece_size` не применяется, работает single-stream fallback (см. [Range-загрузка и fallback](#range-загрузка-и-fallback)).

### Жизненный цикл куска

1. Воркер атомарно забирает следующий `pending` кусок `N`.
2. Открывает Range-запрос `[offset_N, offset_N + piece_size - 1]`.
3. Читает ответ потоком; каждые 64–256 KB тела — вызов `TPieceStorage::write_piece_bytes`. Тело куска целиком в RAM не попадает.
4. После последнего байта проверяет, что принято ровно `piece_size` (иначе кусок возвращается в `pending`).
5. Отправляет `PieceDone` супервизору.

### Коммит каждого куска

Супервизор движка коммитит каждый завершённый кусок сразу через `TPieceStorage::commit_done`. Батчинг коммитов не используется: при piece = 8 МБ реальная частота коммитов — десятки в секунду на файл, что SQLite в `WAL` + `synchronous=NORMAL` держит с большим запасом, а отсутствие буфера в RAM сводит потерю прогресса при `kill -9` к нулю.

Инвариант, который обязана соблюдать любая реализация: **`done` ⇒ байты гарантированно сохранены**. Для `LocalPieceStorage` это `fsync(.data.brook)` + SQLite-транзакция (детали ниже). Для S3 — локальная запись ETag части.

`kill -9` между вызовами `commit_done` — последний (единственный) неподтверждённый кусок остаётся `pending` и на рестарте перекачивается; корректность не страдает.

### Завершение (`LocalPieceStorage::finalize`)

Когда все куски `done`:
1. Финальный `fsync(.data.brook)`.
2. `rename` `<filename>.data.brook` → `<filename>`.
3. Piece-строки загрузки в `brook.db` удаляются (`DELETE FROM pieces WHERE file_id = ?`); переход в `done` пишет `DownloadEngine` через `TQueueStore::update_state`.

### Ресюм и сбои (`LocalPieceStorage`)

Реализация `pending_pieces()` читает статусы из таблицы `pieces` в `./brook.db`:

- **Чистый перезапуск процесса**: читаем `pieces` для `file_id`, докачиваем всё, что не `done`. Runtime-статус `in_progress` не персистится — после рестарта трактуется как `pending`.
- **`.data.brook` удалён или inspect-поля в `file_settings` не совпали с текущими заголовками источника** (сменился `total_size`, `etag`, `last_modified` или число строк в `pieces` не равно ожидаемому `piece_count`): доверять `.data.brook` нечем. `LocalPieceStorage::open` вычищает piece-строки загрузки (`delete_all(id)`), пересоздаёт `.data.brook`, заново преаллоцирует и инициализирует piece-таблицу.
- **`brook.db` повреждена**: это провал уровня демона, а не одной загрузки — `brook server` не поднимется, чиним БД отдельно.

## Схема `brook.db`

Вся persisted-метаинформация демона живёт в одном SQLite-файле `./brook.db` (WAL, `synchronous=NORMAL`, `foreign_keys=ON`). Полный DDL — [schema.dbml](schema.dbml); здесь — смысл таблиц и инварианты, которые нельзя выразить SQL-уровнем:

- **`files`** — одна строка на загрузку: URL, целевая папка, имя файла, текущее состояние (`state_id` → `states.name`).
- **`file_settings`** (1:1 к `files`) — per-download настройки (воркеры, параметры нарезки) и inspect-поля (`total_size`, `piece_size`, `etag`, `last_modified`). Inspect-поля заполняются в `prepare` до первого перехода в `running`.
- **`state_changes`** — история переходов состояний (append-only). При переходе в `failed` поле `reason_code_id` обязательно (ссылается в справочник `reason_codes`); `reason_message` — свободный текст деталей.
- **`pieces`** — карта piece'ов всех активных загрузок; скоуп по `file_id`. В БД живут только `pending` и `done` — runtime-состояние `in_progress` на диск не попадает (после рестарта трактуется как `pending`).
- **`states`**, **`reason_codes`** — справочники с natural-key. Сидятся при миграции `user_version = 1` через `INSERT OR IGNORE`.

Все дочерние таблицы каскадно удаляются при удалении строки из `files` (`ON DELETE CASCADE` на FK). SQL за пределы адаптерного слоя (`SqliteFileRepository`, `SqlitePieceRepository` в `brook-daemon`) не утекает: `brook-core` видит только порты `TQueueStore` / `TPieceStorage`, а `cargo tree -p brook-core` не тянет `rusqlite`.

## Сеть и устойчивость

### Range-загрузка и fallback

- На HEAD / `GET Range: bytes=0-0` подтверждаем: `Accept-Ranges: bytes` + валидный `Content-Range`. Без поддержки Range — fallback: один воркер, без кусков и пре-аллокации, дописываем в файл до EOF.
- На каждом куске ответ должен быть **`206 Partial Content`** с корректным `Content-Range: bytes X-Y/TOTAL`. `200 OK` на Range-запрос — сервер Range-неспособен → fallback.
- HEAD fallback: если HEAD вернул 4xx/5xx или не дал нужных заголовков — пробуем `GET` с `Range: bytes=0-0` (из `Content-Range` достаём размер, сам факт `206` — подтверждение Range).

### Streaming-режим (неизвестный размер)

Если источник не сообщил `Content-Length` (chunked-ответ, прокси, проксирующие CDN), загрузка стартует в streaming-режиме: один воркер, append-mode в `<name>.data.brook`, без преаллокации и без piece-строк в `brook.db`. Прогресс эмитится с `bytes_total = 0` — TUI рендерит indeterminate-гейдж. Resume в этом режиме не поддерживается: после `Pause`/краха возобновление перекачивает всё с начала (источник всё равно отдаёт тело единым потоком от `0`). Финализация — такая же, как у piece-режима: `fsync` + rename `.data.brook → <name>`.

### Эффективный URL после редиректов

`prepare` сохраняет URL, на который источник редиректнул (цепочка 3xx), в `file_settings.effective_url`. Воркеры шлют дальнейшие range-GET'ы именно на этот URL — экономим RTT на повторном резолве подписанных CDN-ссылок и избегаем лишних 3xx на каждом куске. Оригинальный `spec.url` остаётся нетронутым и используется как якорь для повторного inspect'а, если эффективный URL перестал работать (подпись истекла, CDN отозвал токен).

### Защита от мутации файла на сервере

- На первом HEAD / Range-ответе ловим `ETag` (предпочтительно) или `Last-Modified`.
- В каждый последующий Range-запрос шлём `If-Match: <etag>` (или `If-Unmodified-Since`).
- Ответ `412 Precondition Failed` ⇒ источник изменился посреди загрузки. Загрузка помечается `FAILED` с понятной ошибкой (`source mutated`); пользователю — предложение рестартовать с нуля.

### Имя файла

Источник по приоритету: `Content-Disposition: attachment; filename*=` → `filename=` → последний path-сегмент URL → промпт пользователю в модалке `add`.

### Таймауты и ретраи

- **Connect timeout**: 10 s.
- **Read (idle) timeout**: 30 s без новых байт в теле Range-ответа → обрыв соединения + ретрай куска.
- **Retry**: экспоненциальный бэкофф `1s × 2^attempt` + jitter ±20 %, max delay 60 s, max 10 попыток на кусок.
- **Crash-loop guard**: 5 ретраев подряд с одной и той же ошибкой (DNS, 404, 401) ⇒ загрузка помечается `FAILED`, не крутим бесконечно.

### `pwrite` / `read`: короткие операции и `EINTR`

`pwrite(2)` и `read(2)` могут вернуться с частично обработанным буфером (сигнал, редкие ФС). Обёртка — loop до полного слива, пробрасывает только реальные ошибки (`ENOSPC`, `EIO`, и т. п.).

## Процессная безопасность

### Single-instance lock

Два `brook server` в одной CWD — гонка за `brook.db`, глобальную очередь, `.data.brook` в целевой папке. На старте демона — `flock(LOCK_EX | LOCK_NB)` на `.brook.lock` в CWD. Вторая копия — exit с сообщением «brook-daemon already running in this directory». TUI лок не берёт — сколько угодно клиентов могут одновременно подключаться к одному демону.

### Песочница (`target_dir`)

`brook server --directory <ROOT>` канонизирует путь один раз на старте и хранит как `sandbox_root: Arc<PathBuf>`. Любой `spec.target_dir` от клиента (в том числе через proto) проходит через `TPathPolicy::check_target_dir` перед любым I/O:

1. Относительный путь приклеивается к корню (`root.join(target_dir)`), чтобы `canonicalize` не резолвил его относительно CWD демона.
2. Канонизируется самый длинный существующий префикс (через `fs::canonicalize`); оставшийся «виртуальный» хвост прикладывается лексически с обработкой `.`/`..`. Это ловит и симлинки в любой точке цепочки, и `../..`-эскейпы над несуществующим путём.
3. Результат обязан `starts_with(sandbox_root)` — иначе `core::Error::PathEscapesRoot { attempted }` → gRPC `PermissionDenied`.

Песочница действует на всё, что пишет демон: целевой файл, `.data.brook`, любые вспомогательные артефакты. `--directory` — обязательный флаг без дефолта, чтобы случай «забыл ограничить» не случался молча; для автоспавна TUI подставляет `~/Downloads`.

### Аутентификация (bearer)

Опциональный пароль клиента живёт только в памяти демона (`Runtime.client_pass: Option<Arc<String>>`), не пишется в `brook.db` и в `brook.yaml`. `--client-pass` передаётся CLI-флагом или через env `BROOK_CLIENT_PASS`.

- **Сервер** — `brook-api::AuthInterceptor` включает `trace_interceptor` (корреляция) и, если задан ожидаемый токен, сверяет заголовок `authorization: bearer <pass>` через `subtle::ConstantTimeEq`. Без токена или с неверным — `Unauthenticated`.
- **Клиент** (TUI) — `brook-tui::connect::BearerInterceptor` кладёт тот же заголовок в каждый unary/streaming-запрос. Тип `AuthedChannel = InterceptedService<Channel, BearerInterceptor>` протащен через весь клиентский код, так что забыть приложить пароль невозможно.
- **Non-loopback guard** — `build_runtime` отказывается биндиться, если `host.is_loopback() == false` и `--client-pass` не задан. Это правило срабатывает до `TcpListener::bind`: без пароля наружу нельзя даже случайно. `0.0.0.0` и `::` считаются не-loopback корректно (`is_unspecified()` ≠ `is_loopback()`).

### Проверки перед стартом загрузки

- `statvfs` на целевую ФС: свободного места ≥ размер файла. Иначе — `FAILED` с понятной ошибкой, без попытки `F_PREALLOCATE`.
- `TPathPolicy::check_target_dir` на `spec.target_dir` (см. [Песочница](#песочница-target_dir)) — до любого I/O.

### Endpoint-файл (`.brook.endpoint`)

После успешного `bind` `brook server` атомарно пишет `.brook.endpoint` в CWD: JSON `{ host, port }` — это позволяет TUI подхватить даже ephemeral-порт (`--port 0`) без хардкода. Формат и атомарная запись (tempfile + rename в той же директории) живут в `brook-runtime::endpoint`, константы (имя файла, дефолтный порт, имя заголовка, bearer-префикс) — в `brook-runtime::constants`. На graceful shutdown демон удаляет файл; если сайдкар остался от мёртвого процесса, TUI удаляет его перед автоспавном.

### Graceful shutdown

`SIGTERM` / `SIGINT` в демоне:
1. Прекращаем приём новых команд через gRPC.
2. Пауза всех `DownloadEngine` — каждый доводит in-flight куски до batch-границы.
3. Финальный `fsync(.data.brook)` + batch-коммит piece-строк в `brook.db` для каждой активной загрузки.
4. Закрываем gRPC-сервер, `./brook.db`, лог-writer, удаляем `.brook.endpoint`, отпускаем `.brook.lock`.
5. Exit 0.

В TUI `q` открывает модалку `QuitConfirm`. Если TUI сам поднимал демон (`can_stop_daemon = true`), предлагается пункт «остановить демон» — он шлёт `Shutdown` RPC и ждёт закрытия стрима; иначе пункт скрыт и клиент просто закрывается, демон продолжает работать. Закрытие клиента при активных загрузках не останавливает их, если пункт «stop daemon» не выбран.

Без graceful shutdown на ctrl-c демона — теряем до 16 `done`-кусков трафика на каждую активную загрузку (перекачаются после рестарта).

## API-слой (`brook-api`)
- Генерируется из `.proto` через `tonic-build` в build.rs `brook-proto`.
- Тонкая обёртка вокруг `DownloadManager`: proto ↔ core.
- Запускается как фоновая задача серверного стека (подкоманда `brook server`).
- Контракт — [api.md](api.md).

### Поток событий и Watch

- `DownloadEngine` внутри себя агрегирует счётчики в таймере 200 ms — эмитит один `ProgressTick` за окно. Смены лайфсайкла проходят мимо таймера и уходят мгновенно.
- `DownloadManager` держит два независимых центральных канала — `broadcast::Sender<FileLifecycleEvent>` и `broadcast::Sender<ProgressEvent>` (ring 1024 каждый). Это единственные точки, откуда питаются `WatchFile` и `WatchProgress` соответственно.
- На каждого watch-клиента `brook-api` спавнит fanout-задачу:
  1. Для `WatchFile` при коннекте — эмитит `snapshot` по каждому известному файлу (initial state). `WatchProgress` initial-sync'а не делает: актуальный снимок живёт в `WatchFile`, прогресс начинается с первого тика активных файлов.
  2. Читает соответствующий центральный `broadcast`, конвертирует в proto (`FileEvent` / `ProgressTick`), пишет в tonic tx.
  3. `tx.send().await` обеспечивает транспортный backpressure: при медленном клиенте fanout блокируется, broadcast-receiver отстаёт.
  4. При `RecvError::Lagged(n)` в `WatchFile` — запрашивает у `DownloadManager` свежие snapshot'ы всех активных файлов и шлёт их клиенту, затем продолжает нормальный цикл. В `WatchProgress` `Lagged` логируется и игнорируется: следующий тик перепишет UI.
- Гарантия для клиента: сколько бы `ProgressTick`-ов он ни потерял, **state всегда догоняется** через `snapshot` в `WatchFile`.
- Протокольные детали (какое событие когда, что перезаписывает) — [api.md#watchfile-и-watchprogress-события-и-реконсиляция](api.md#watchfile-и-watchprogress-события-и-реконсиляция).

## UI-слой (`brook-tui`, бинарь `brook`)
- `ratatui` + `crossterm`.
- gRPC-клиент (`tonic::Channel`, обёрнутый в `InterceptedService` с `BearerInterceptor`) к демону. Адрес: `--remote HOST:PORT` > sidecar `.brook.endpoint` в CWD > дефолтный loopback-порт. `--port` перекрывает sidecar.
- Две точки взаимодействия:
  - Команды — unary (`add`, `pause`, ...).
  - События — две параллельные подписки на server-streaming (`WatchFile` + `WatchProgress`); фоновые задачи питают view-model.
- View-model мутируется **только** в ответ на событие от API.
- UX-детали — [ux.md](ux.md).

## Конфигурация

Вся конфигурация живёт в YAML-файле `./brook.yaml` рядом с `brook.db` и `.brook.lock`. В SQLite её нет — `brook.db` хранит только очередь загрузок. При первом старте `brook server` создаёт `brook.yaml` с дефолтами и комментариями. На каждом старте демон читает файл и применяет значения в `DownloadManager` и `brook-api`. CLI-флаги `brook server` (`--host`, `--port`, `--client-pass`, `--directory`) перекрывают соответствующие значения YAML. Изменения на лету не поддерживаются — правки требуют рестарта демона.

Ключи делятся на два класса:

- **Global-only** — действуют на весь процесс: порт gRPC, лимит параллельности, настройки логирования, политика дублирующихся URL. Политики конфликта имени файла нет: демон на совпадение имени возвращает `AlreadyExists`, а клиент (TUI) сам ретраит `Add` со свободным `<stem> (N).<ext>`.
- **Per-download defaults** — YAML задаёт дефолт, но клиент в `DownloadSpec` может прислать override. Это число воркеров, параметры нарезки piece'ов и целевой каталог.

Ключи и дефолты:

| key                           | default               | override в `DownloadSpec` | назначение                        |
|-------------------------------|-----------------------|---------------------------|-----------------------------------|
| `download.max_concurrent`     | `3`                   | —                         | одновременно активных загрузок    |
| `download.piece_target_count` | `128`                 | —                         | целевое макс. число кусков на файл|
| `download.piece_size_min_mib` | `16`                  | —                         | минимальный размер куска, MiB     |
| `download.piece_size_max_mib` | `128`                 | —                         | максимальный размер куска, MiB    |
| `download.default_dir`        | `~/Downloads`         | `target_dir` (пустой → дефолт) | дефолтная папка назначения   |
| `download.on_duplicate_url`   | `ask`                 | —                         | `ask` / `skip` / `add`            |
| `api.port`                    | `7090`                | —                         | порт gRPC                         |
| `api.bind`                    | `127.0.0.1`           | —                         | bind-адрес; не-loopback требует `--client-pass` |
| `log.dir`                     | `~/Library/Logs/brook`| —                         | каталог JSON-логов                |
| `log.rotate_count`            | `10`                  | —                         | файлов в ротации                  |
| `log.rotate_size_mb`          | `50`                  | —                         | размер файла до ротации           |

`~` в путях раскрывается в `$HOME` на старте. Невалидное значение → ошибка старта `brook server` с явным сообщением и именем ключа. Неизвестный ключ — ошибка парса (strict mode), чтобы опечатки не проходили молча.

Для каждой загрузки демон собирает **effective-конфиг нарезки**: берёт дефолты из YAML и перекрывает полями `DownloadSpec` там, где клиент их прислал. Степень двойки, `min ≤ max`, `target_count ≥ 1` валидируются в том же месте.

## Словарь статусов

Все «живые» сущности — файл, воркер, piece, attempt — используют единый справочник из семи значений: `pending`, `running`, `paused`, `retrying`, `done`, `failed`, `cancelled`. Таблица `statuses` в `brook.db` хранит эти значения как natural-key, а колонки статуса у каждой сущности добавляют `CHECK` на допустимое для неё подмножество. В Rust те же подмножества выражены отдельными enum'ами (`FileStatus`, `WorkerStatus`, `PieceStatus`, `AttemptStatus`); сериализация в строку (`as_str`) и парсинг (`FromStr`) — единый контракт с БД.

Смысл: запрос в аналитике вроде «посчитай все failed» работает единообразно в любой таблице, а компилятор страхует от присваивания «чужого» статуса (например, `retrying` у piece'а — невалидно и по CHECK, и по типу).

## Идентичность воркера и журнал попыток

Воркер — единица параллелизма **одной engine-сессии**. У каждой загрузки фиксированный набор слотов (`workers` из `file_settings`); engine при старте создаёт ровно столько строк в `workers` со свежими UUID'ами и `slot_index` `0..N-1`. Эти UUID'ы живут только до паузы или терминального перехода: если потом engine поднимается заново (resume, retry, рестарт демона), старые строки переводятся в `paused`, новые строки получают свежие идентификаторы. Пересечения `worker_id` между сессиями невозможны, поэтому GROUP BY `worker_id` в аналитике всегда считает историю ровно одной попытки одной сессии.

Таблица `piece_attempts` — журнал попыток скачать конкретный piece конкретным воркером. Одна попытка = одна строка со своим `started_at`/`finished_at`/`bytes`/`error`. Ретраи после транзиентных ошибок создают новую строку, а не перезаписывают старую — это источник правды для per-worker и per-piece метрик (скорость, доля failed, число ретраев). Суммирующих полей у `workers` нет: всё выводится запросом из `piece_attempts`.

Старт демона проводит одноразовый recovery-sweep под `.brook.lock`: любые `running`-воркеры и `running`-attempt'ы, оставшиеся от предыдущего (возможно, упавшего) инстанса, переводятся в `paused`. Это даёт чистый инвариант: внутри одной живой сессии демона в таблице не может быть ни одного «осиротевшего running».

## Наблюдаемость

- `tracing` + JSON-форматтер во всех крейтах.
- `session_id` — xid, создаётся при старте процесса; прокидывается как поле во всех root-спанах.
- `download_id` — у каждой загрузки; `brook-core` входит в `span!(download_id = ...)` при любых операциях.
- gRPC-запросы получают `request_id`; если касаются конкретной загрузки — добавляется `download_id`.
- Лог-файлы: `~/Library/Logs/brook/brook-<session_id>.jsonl`. Ротация — [open question](open-questions.md).

## Тестирование
- `brook-core` — юнит + интеграционные без сети (`wiremock` или локальный тест-сервер). Отдельный тест: **пиковый RSS при длительной загрузке не растёт с размером файла** (проверка записи по кускам).
- `brook-api` — интеграционные: tonic-server + tonic-client в одном процессе, проверка контракта.
- `brook` — ручная прогонка квалификационных сценариев (см. [open-questions.md](open-questions.md)).
