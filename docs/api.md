...
- Локальный UI (`brook`, крейт `brook-tui`) — отдельный процесс, ходит в демон (`brook server`) только через gRPC. «Короткого пути» в обход API нет даже локально.
...
- Settings — не в API в MVP; правятся в `brook.yaml` и подхватываются при следующем старте демона.
